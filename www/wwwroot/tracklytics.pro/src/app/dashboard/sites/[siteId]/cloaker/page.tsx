  ...
  React.useEffect(() => {
    async function loadSettings() {
      ...
      try {
        ...
        // Ensure default values are applied for any missing keys
        setState(prev => ({...defaultState, ...settings}));
      } catch (error: any) {
    ...
    <div className="space-y-6">
                        ...
                        <TableBody>
                            {paginatedRules.length > 0 ? paginatedRules.map(rule => (
                                <TableRow key={rule.id}>
                                    ...
                                    <TableCell className="text-right"><Button variant="ghost" size="icon"><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" onClick={() => handleDeleteRule(rule.id)}><Trash2 className="h-4 w-4" /></Button></TableCell>
                                </TableRow>
        ...
        <Card>
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-4 rounded-lg border p-4"><div className="flex items-center justify-between"><div className="flex items-start gap-4"><Laptop className="h-6 w-6 text-primary mt-1 hidden sm:block" /><div className="space-y-0.5"><Label htmlFor="os-filter-switch" className="text-base font-semibold">Filtro por Sistema Operacional</Label><p className="text-sm text-muted-foreground">Filtre visitantes com base no sistema operacional.</p></div></div><Switch id="os-filter-switch" checked={state.osFilterEnabled} onCheckedChange={v => handleStateChange('osFilterEnabled', v)} /></div>{state.osFilterEnabled && (<div className="space-y-4 pt-4 border-t mt-4"><div className="w-full max-w-xs space-y-2"><Label htmlFor="os-action">Ação</Label><Select value={state.osFilterAction} onValueChange={v => handleStateChange('osFilterAction', v)}><SelectTrigger id="os-action"><SelectValue placeholder="Selecione a ação" /></SelectTrigger><SelectContent><SelectItem value="block">Bloquear Selecionados</SelectItem><SelectItem value="allow">Permitir Apenas</SelectItem></SelectContent></Select></div><div><Label>Selecione os Sistemas Operacionais</Label><div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 pt-2">{osOptions.map((os) => (<div key={os.id} className="flex items-center space-x-2"><Checkbox id={`os-${os.id}`} checked={state.selectedOs[os.id]} onCheckedChange={checked => handleStateChange('selectedOs', {...state.selectedOs, [os.id]: !!checked})} /><Label htmlFor={`os-${os.id}`} className="font-normal">{os.label}</Label></div>))}</div></div></div>)}</div>
